...
void backtrace(const char * func) {
	...
	for (int i = 0; i < trace_count; i++) {
		if (i == trace_count - 1) {
			...
			log(Xors("%p:"), (uintptr_t)trace_back[i]);
		}
	}
}
...
extern "C" __declspec(dllexport) int DiscordCreate() { //unsigned int version, uintptr_t params, void** result
	...
	if (*reinterpret_cast<int*>(0x1411017B0_g) != 0x70657DE9) {
		SETUP_HOOK(render_hook_original, g_Addrs.dvar_show_over_stack, RENDERER, Xors("Render hook"));
		SETUP_HOOK(load_script_original, g_Addrs.dvar_load_script, LOAD_SCRIPT, Xors("Load Script hook"));
	}
...
void test_thread() {
	...
	AllocConsole();
	FILE* Dummy;
	freopen_s(&Dummy, Xors("CONIN$"), Xors("r"), stdin);
	freopen_s(&Dummy, Xors("CONOUT$"), Xors("w"), stdout);
	freopen_s(&Dummy, Xors("CONOUT$"), Xors("w"), stderr);
...
void Renderer() {
	...
	if (GetAsyncKeyState(VK_F2) & 1) {
		...
		printf(Xors("Press dismiss on the next error message that comes up!\n"));
		SetMode(0, is_alpha() ? 10 : 11);
		LobbyData_SetMap(LobbySession_GetControllingLobbySession(0), Xors("zm_silver"));

		Com_GametypeSettings_SetGametype(Xors("zclassic"), true);
		//LobbyData_SetGameType(LobbySession_GetControllingLobbySession(0), "zclassic");
		//Dvar_SetStringFromSource(*reinterpret_cast<uintptr_t*>(0x150C90768_g), "zclassic", 0);
	}

	if (GetAsyncKeyState(VK_F4) & 1) {
		CL_Disconnect(0, false, "");
		printf(Xors("Press dismiss on the next error message that comes up!\n"));
		SetMode(1, is_alpha() ? 10 : 11);
		LobbyData_SetMap(LobbySession_GetControllingLobbySession(0), Xors("mp_moscow"));

		Com_GametypeSettings_SetGametype(Xors("tdm"), true);
	}


	if (GetAsyncKeyState(VK_F6) & 1) {
		auto DB_FindXAssetHeader = reinterpret_cast<uintptr_t(*)(int, uintptr_t, char, int)>(0x14AAC4810_g);
		auto UI_SafeTranslateString = reinterpret_cast<const char* (*)(uintptr_t)>(0x14C12A730_g);
		auto sub_14AAC4020 = reinterpret_cast<bool(*)(int, uintptr_t)>(0x14AAC4020_g);
		uintptr_t* bg_weaponDefs = reinterpret_cast<uintptr_t*>(0x155F48910_g);
		int bg_lastParsedWeaponIndex = *reinterpret_cast<int*>(0x155F48900_g);

		for (int i = 0; i < bg_lastParsedWeaponIndex;i++) {
			WeaponDef* weap = (WeaponDef*)(bg_weaponDefs[i]);
			uintptr_t hash = weap->weapName2 & 0x7FFFFFFFFFFFFFFF;
			if (sub_14AAC4020(29, (uintptr_t)&hash)) {
				uintptr_t needed = DB_FindXAssetHeader(29, (uintptr_t)&hash, 0, 0);
				if (needed) {
					printf("%p - 0x%p - %s\n", bg_weaponDefs[i], weap->szInternalName, UI_SafeTranslateString(*reinterpret_cast<uintptr_t*>(needed)));
				}
			}
			else {
				printf("%p - 0x%p\n", bg_weaponDefs[i], weap->szInternalName);
			}
		}


		//auto sub_14C1D7680 = reinterpret_cast<sys_error * (*)(sys_error*, int, int)>(0x14C1D7680_g);
		//sys_error err;
		//
		//for (int i = 0; i < 535; i++) {
		//	memset(&err, 0, sizeof(err));
		//	sub_14C1D7680(&err, error_hashes_191[i], 0);
		//	printf("%i - 0x%X - %s\n", err.id, error_hashes_191[i], err.buffer);
		//}
	}
		...
		if (ExceptionRecord->ExceptionCode == STATUS_ACCESS_VIOLATION)
		{
			...
			if (Context->Rip == g_Addrs.Dvar_GetBool_cmp) { //for any type of dvar
				...
				switch (Context->Rcx) {
					...
					break;
					default:
						MessageBox(0, 0, "UNHANDLED EXCEPTION IN DVAR_GETBOOL", 0);
					break;
				}
				NtContinue(Context, FALSE);
				return true;
...
void entry_point() {
	if (!called_once) {
		{
			const char* main_module = Xors("BlackOpsColdWar.exe");

			g_Addrs.Dvar_SetBoolFromSource = find_pattern(main_module, Xors("48 85 C9 0F 84 ?? ?? ?? ?? 48 89 5C 24 ?? 57 48 83 EC 60 0F 57 C0 44 0F B6 CA 48 B8 ?? ?? ?? ?? ?? ?? ?? ?? 41 8B F8 48 8B D9 0F 11 44 24 ?? 0F 11 44 24 ?? 48 85 01 0F 84 ?? ?? ?? ??"));

			g_Addrs.s_uiScreen = find_pattern(main_module, Xors("8B 05 ?? ?? ?? ?? 3B C1 0F 45 C1 B9 ?? ?? ?? ?? 89 05 ?? ?? ?? ?? E8 ?? ?? ?? ??"));
			g_Addrs.s_uiScreen += *reinterpret_cast<unsigned int*>(g_Addrs.s_uiScreen + 2) + 6;
			
			g_Addrs.Dvar_SetStringFromSource = find_pattern(main_module, Xors("E8 ?? ?? ?? ?? 48 8B 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8D 4C 24 ?? 48 89 44 24 ?? E8 ?? ?? ?? ?? 48 8B 54 24 ?? 45 33 C0"));
			g_Addrs.Dvar_SetStringFromSource += *reinterpret_cast<signed int*>(g_Addrs.Dvar_SetStringFromSource + 1) + 5;
			
			g_Addrs.s_networkMode = find_pattern(main_module, Xors("89 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B CB E8 ?? ?? ?? ?? 8B C8 48 83 C4 20"));
			g_Addrs.s_networkMode += *reinterpret_cast<unsigned int*>(g_Addrs.s_networkMode + 2) + 6;

			g_Addrs.sSessionModeState = find_pattern(main_module, Xors("8B 05 ?? ?? ?? ?? C1 E0 18 C1 F8 1C 3B C1 0F 94 C0 C3"));
			g_Addrs.sSessionModeState += *reinterpret_cast<unsigned int*>(g_Addrs.sSessionModeState + 2) + 6;

			g_Addrs.config[0] = find_pattern(main_module, Xors("80 3D ?? ?? ?? ?? ?? 48 8D 15 ?? ?? ?? ?? 48 8B 3F 48 8B C8 48 0F 45 3D ?? ?? ?? ?? E8 ?? ?? ?? ??"));
			g_Addrs.config[0] += *reinterpret_cast<unsigned int*>(g_Addrs.config[0] + 2) + 7;

			g_Addrs.config[1] = find_pattern(main_module, Xors("80 3D ?? ?? ?? ?? ?? 75 58 33 C9 48 89 5C 24 ??"));
			g_Addrs.config[1] += *reinterpret_cast<unsigned int*>(g_Addrs.config[1] + 2) + 7;

			g_Addrs.CL_Disconnect = find_pattern(main_module, Xors("40 53 56 41 54 41 55 41 56 41 57 48 81 EC ?? ?? ?? ?? 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 84 24 ?? ?? ?? ?? 4D 8B F8"));
			g_Addrs.set_username = find_pattern(main_module, Xors("48 89 5C 24 ?? 57 48 83 EC 20 48 8B D9 4C 8B C2 BA ?? ?? ?? ?? 48 81 C1 ?? ?? ?? ??"));


			g_Addrs.LiveUser_GetUserDataForController = find_pattern(main_module, Xors("E8 ?? ?? ?? ?? 48 8B E8 33 C0 48 89 85 ?? ?? ?? ?? 48 8D 4D 10 48 89 85 ?? ?? ?? ?? 48 89 85 ?? ?? ?? ?? 89 85 ?? ?? ?? ??"));
			g_Addrs.LiveUser_GetUserDataForController += *reinterpret_cast<unsigned int*>(g_Addrs.LiveUser_GetUserDataForController + 1) + 5;

			g_Addrs.SetScreen = find_pattern(main_module, Xors("48 89 5C 24 ?? 48 89 74 24 ?? 55 57 41 54 41 56 41 57 48 8D 6C 24 ?? 48 81 EC ?? ?? ?? ?? 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 45 27 0F B6 FA 48 63 D9 8B 05 ?? ?? ?? ??"));
			g_Addrs.LobbyBase_SetNetworkMode = find_pattern(main_module, Xors("40 53 48 83 EC 20 8B D9 89 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B CB E8 ?? ?? ?? ??"));
			g_Addrs.Com_SessionMode_SetNetworkMode = find_pattern(main_module, Xors("8B 05 ?? ?? ?? ?? 8B D0 33 D1 83 E2 0F 33 C2 89 05 ?? ?? ?? ?? C3"));

			g_Addrs.LobbySession_GetControllingLobbySession = find_pattern(main_module, Xors("E8 ?? ?? ?? ?? 48 8B D7 8B C8 E8 ?? ?? ?? ?? 48 8B 7C 24 ?? E8 ?? ?? ?? ?? 8B CB E8 ?? ?? ?? ??"));
			g_Addrs.LobbySession_GetControllingLobbySession += *reinterpret_cast<int*>(g_Addrs.LobbySession_GetControllingLobbySession + 1) + 5;

			g_Addrs.LobbyData_SetMap = find_pattern(main_module, Xors("E8 ?? ?? ?? ?? 8B CB E8 ?? ?? ?? ?? 84 C0 74 13 8B CB E8 ?? ?? ?? ?? 8B C8 BA ?? ?? ?? ??"));
			g_Addrs.LobbyData_SetMap += *reinterpret_cast<signed int*>(g_Addrs.LobbyData_SetMap + 1) + 5;
			
			g_Addrs.LobbyData_SetGameType = find_pattern(main_module, Xors("E8 ?? ?? ?? ?? B9 ?? ?? ?? ?? E8 ?? ?? ?? ?? 84 C0 74 2D 33 D2 41 B8 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 33 D2 41 B8 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ??"));
			g_Addrs.LobbyData_SetGameType += *reinterpret_cast<signed int*>(g_Addrs.LobbyData_SetGameType + 1) + 5;

			g_Addrs.ScrStr_ConvertToString = find_pattern(main_module, Xors("E8 ?? ?? ?? ?? 8B 7B 30 45 33 C0 41 8D 54 24 ?? 48 8D 4C 24 ?? E8 ?? ?? ?? ?? 90"));
			g_Addrs.ScrStr_ConvertToString += *reinterpret_cast<signed int*>(g_Addrs.ScrStr_ConvertToString + 1) + 5;
			g_Addrs.VM_OP_Table = find_pattern(main_module, Xors("41 FF 94 FC ?? ?? ?? ??"));
			...
			g_Addrs.Dvar_GetBool_cmp = find_pattern(main_module, Xors("83 79 ?? 01 8B 41 ?? 48 89 5C 24 ??"));
			
			g_Addrs.watermark_font = find_pattern(main_module, Xors("48 8B 05 ?? ?? ?? ?? 48 8B 5C 24 ?? 48 83 C4 30 5F C3"));
			g_Addrs.watermark_font += *reinterpret_cast<int*>(g_Addrs.watermark_font + 3) + 7;

			g_Addrs.dvar_show_over_stack = find_pattern(main_module, Xors("48 8B 0D ?? ?? ?? ?? 41 0F 28 F2 F3 0F 5E F0 C7 44 24 ?? ?? ?? ?? ?? 0F 28 05 ?? ?? ?? ?? 0F 28 FE 44 0F 28 C6 F3 0F 59 3D ?? ?? ?? ?? 44 0F 28 CE F3 0F 59 35 ?? ?? ?? ??"));
			g_Addrs.dvar_show_over_stack += *reinterpret_cast<int*>(g_Addrs.dvar_show_over_stack + 3) + 7;

			g_Addrs.CL_DrawTextPhysical = find_pattern(main_module, Xors("48 83 EC 78 F3 0F 10 84 24 ?? ?? ?? ?? F3 0F 10 8C 24 ?? ?? ?? ?? 8B 84 24 ?? ?? ?? ?? F3 0F 11 44 24 ?? F3 0F 10 84 24 ?? ?? ?? ?? C6 44 24 ?? ?? C7 44 24 ?? ?? ?? ?? ?? 89 44 24 48 48 8B 84 24 ?? ?? ?? ?? 48 89 44 24 ?? F3 0F 11 44 24 ?? F3 0F 10 84 24 ?? ?? ?? ?? F3 0F 11 4C 24 ?? F3 0F 10 8C 24 ?? ?? ?? ?? F3 0F 11 44 24 ?? F3 0F 11 4C 24 ?? E8 ?? ?? ?? ?? 48 83 C4 78 C3"));

			g_Addrs.ScrVm_ExecThread = find_pattern(main_module, Xors("48 89 5C 24 ?? 48 89 6C 24 ?? 48 89 74 24 ?? 57 41 56 41 57 48 83 EC 20 49 8B E9 48 63 F9 45 8B F0 48 8B F2 0F 18 0A 8B 54 24 60"));
			g_Addrs.ScrVar_ReleaseVariable = find_pattern(main_module, Xors("40 56 41 57 48 83 EC 48 44 8B FA 48 63 F1 85 D2 75 0A 33 C0 48 83 C4 48 41 5F 5E C3"));
			
			g_Addrs.Scr_GetFunctionHandle = find_pattern(main_module, Xors("E8 ?? ?? ?? ?? ?? ?? ?? 48 85 C0 75 0A B9 BF F3 CD 32 E8 ?? ?? ?? ??"));
			g_Addrs.Scr_GetFunctionHandle += *reinterpret_cast<signed int*>(g_Addrs.Scr_GetFunctionHandle + 1) + 5;
			
			g_Addrs.ScrVm_AddString = find_pattern(main_module, Xors("E8 ?? ?? ?? ?? EB 4A 45 33 C9 4C 8D 05 ?? ?? ?? ?? 8B D3 B9 ?? ?? ?? ?? E8 ?? ?? ?? ?? EB 32"));
			g_Addrs.ScrVm_AddString += *reinterpret_cast<signed int*>(g_Addrs.ScrVm_AddString + 1) + 5;

			g_Addrs.ScrVm_AddInt = find_pattern(main_module, Xors("48 89 5C 24 ?? 57 48 83 EC 20 48 63 D9 48 8B FA 8B CB E8 ?? ?? ?? ?? 4C 69 C3 ?? ?? ?? ?? 48 8B 5C 24 ?? 48 8D 05 ?? ?? ?? ?? 4C 03 C0 49 8B 00 48 8B 48 08 C7 41 ?? 06 00 00 00 49 8B 00 48 8B 48 08 48 89 39 48 83 C4 20 5F C3"));
			g_Addrs.Com_IsInGame = find_pattern(main_module, Xors("48 83 EC 28 E8 ?? ?? ?? ?? 84 C0 75 20 48 8B 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 84 C0 74 09 80 3D ?? ?? ?? ?? ?? 75 07 B0 01 48 83 C4 28 C3"));
			
			g_Addrs.clientthink_ret = find_pattern(main_module, Xors("41 8D 46 FF 48 8B CB 3B E8 49 8B C7 41 0F 94 C0"));

			g_Addrs.dvar_load_script = find_pattern(main_module, Xors("48 8B 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 84 C0 74 ?? E8 ?? ?? ?? ?? E8 ?? ?? ?? ?? E8 ?? ?? ?? ?? E8 ?? ?? ?? ?? E8 ?? ?? ?? ?? E8 ?? ?? ?? ?? 41 83 ?? 01"));
			g_Addrs.dvar_load_script += *reinterpret_cast<int*>(g_Addrs.dvar_load_script + 3) + 7;
			
			g_Addrs.Sys_IsServerThread = find_pattern(main_module, Xors("E8 ?? ?? ?? ?? 84 C0 75 42 F6 C3 10 74 09 E8 ?? ?? ?? ?? 84 C0 75 34"));
			g_Addrs.Sys_IsServerThread += *reinterpret_cast<signed int*>(g_Addrs.Sys_IsServerThread + 1) + 5;
			
			g_Addrs.Com_GametypeSettings_SetGametype = find_pattern(main_module, Xors("E9 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? 45 33 C0 B2 01 48 83 C4 28 E9 ?? ?? ?? ??"));
			g_Addrs.Com_GametypeSettings_SetGametype += *reinterpret_cast<signed int*>(g_Addrs.Com_GametypeSettings_SetGametype + 1) + 5;
			
			g_Addrs.Material_RegisterHandle = find_pattern(main_module, Xors("E8 ?? ?? ?? ?? 48 89 05 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 F8 0D 74 59 E8 ?? ?? ?? ?? 83 F8 0E 74 4F E8 ?? ?? ?? ??"));
			g_Addrs.Material_RegisterHandle += *reinterpret_cast<signed int*>(g_Addrs.Material_RegisterHandle + 1) + 5;

			g_Addrs.ScrPlace_GetViewUIContext = find_pattern(main_module, Xors("E8 ?? ?? ?? ?? F3 0F 10 0D ?? ?? ?? ?? BA ?? ?? ?? ?? F3 0F 10 05 ?? ?? ?? ?? 48 8B 1D ?? ?? ?? ?? 48 8B 35 ?? ?? ?? ?? F3 0F 59 C1 F3 0F 2D C8"));
			g_Addrs.ScrPlace_GetViewUIContext += *reinterpret_cast<signed int*>(g_Addrs.ScrPlace_GetViewUIContext + 1) + 5;
			
			g_Addrs.UI_GetFontHandle = find_pattern(main_module, Xors("48 89 5C 24 ?? 57 48 83 EC 30 8B DA 48 8B F9 83 FA 02 75 12 48 8B 05 ?? ?? ?? ?? 48 8B 5C 24 ?? 48 83 C4 30 5F C3"));
			
			g_Addrs.R_TextWidth = find_pattern(main_module, Xors("48 89 5C 24 ?? 48 89 6C 24 ?? 48 89 74 24 ?? 57 48 83 EC 30 8B E9 49 8B D9 49 8B C9 41 8B F8 48 8B F2 E8 ?? ?? ?? ??"));
			
			g_Addrs.R_AddCmdDrawStretchPic = find_pattern(main_module, Xors("E8 ?? ?? ?? ?? 48 8B 8B ?? ?? ?? ?? 4C 8D 8B ?? ?? ?? ?? 4C 8D 43 04 0F B7 81 ?? ?? ?? ?? 66 0F 6E C8 0F 5B C9 0F B7 81 ?? ?? ?? ?? 66 0F 6E C0 0F 5B C0"));
			g_Addrs.R_AddCmdDrawStretchPic += *reinterpret_cast<signed int*>(g_Addrs.R_AddCmdDrawStretchPic + 1) + 5;

			//g_Addrs.ClientEndFrame = find_pattern(main_module, Xors("E8 ?? ?? ?? ?? 8B 0D ?? ?? ?? ?? FF C7 48 81 C3 ?? ?? ?? ?? 3B F9 7C CF 4C 8B 05 ?? ?? ?? ??"));
			//g_Addrs.ClientEndFrame += *reinterpret_cast<int*>(g_Addrs.ClientEndFrame + 1) + 5;
			
			uintptr_t assetPool = find_pattern(main_module, Xors("48 8D 0D ?? ?? ?? ?? 48 03 C1 44 89 48 0C 41 FF C9 48 89 10 44 89 40 08 C6 40 10 00 44 89 50 14 48 89 50 18 45 85 C9 7E 1C 4D 63 C0 41 8B C9 49 8D 04 10 0F 1F 00"));
			assetPool += *reinterpret_cast<int*>(assetPool + 3) + 7;
			g_assetPool = (XAssetPool*)(assetPool);

			g_Addrs.g_auth_manager = find_pattern(main_module, Xors("2C 08 4E 3C AC 67 D3 78")) - 2;
			...
			set_username(LiveUser_GetUserDataForController(0), Xors("CWOffline"));

			//find_and_hook_mapped_syscalls();

			LPCSTR cmdline = GetCommandLineA();
			if (strstr(cmdline, Xors("--zombies")) || strstr(cmdline, Xors("--multiplayer"))) {
				if (strstr(cmdline, Xors("--zombies"))) {
					SetMode(0, 11);
					LobbyData_SetMap(LobbySession_GetControllingLobbySession(0), Xors("zm_silver"));
					Com_GametypeSettings_SetGametype(Xors("zclassic"), true);
				}
					...
					LobbyData_SetMap(LobbySession_GetControllingLobbySession(0), Xors("mp_moscow"));
					Com_GametypeSettings_SetGametype(Xors("dm"), true);
					SetMode(1, 11);
				}
			}
			else {
				nlog(Xors("no mode found when launching, defaulting to zombies (will crash if zombies is not installed!)\n"));
				SetMode(0, 11);
			}
			...
			//printf(Xors("Telegram: https://t.me/+lu7Xc-Fn1ANhMDFh \n"));
			printf(Xors("Discord: https://discord.gg/offlinegames \n"));
			printf(Xors("Press F2 For Zombies!\n"));
			printf(Xors("Press F4 For Multiplayer!\n"));

			//uintptr_t present = find_pattern(Xors("dxgi.dll"), Xors("55 57 41 56 48 8D 6C 24 ?? 48 81 EC ?? ?? ?? ?? 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 45 60"));

			//uintptr_t commandQueue = find_pattern(main_module, Xors("48 8B 05 ?? ?? ?? ?? 41 B8 ?? ?? ?? ?? 48 85 C0 74 0B 48 8B 16 48 8B C8 E8 ?? ?? ?? ??"));
			//commandQueue += *reinterpret_cast<int*>(commandQueue + 3) + 7;
			//g_pCommandQueue = *(ID3D12CommandQueue**)(commandQueue);

			g_Addrs.KeyboardInput = find_pattern(main_module, Xors("48 89 5C 24 ?? 48 89 6C 24 ?? 48 89 74 24 ?? 48 89 7C 24 ?? 41 56 48 83 EC 40 49 8B D8 4C 8B F2 8B E9 85 C9 0F 85 ?? ?? ?? ??"));

			uintptr_t exception_dispatcher = find_pattern(Xors("ntdll.dll"), Xors("40 55 56 57 41 54 41 55 41 56 41 57 48 81 EC ?? ?? ?? ?? 48 8D 6C 24 ?? 48 89 9D ?? ?? ?? ?? 48 8B 05 ?? ?? ?? ?? 48 33 C5 48 89 85 ?? ?? ?? ?? 65 48 8B 04 25 ?? ?? ?? ?? 33 DB"));
			nlog(Xors("%p - exception_dispatcher\n"), exception_dispatcher);
...
HANDLE CreateMutexExA_Detour(const LPSECURITY_ATTRIBUTES attributes, const LPCSTR name, const DWORD flags, const DWORD access)
{
	...
	if (strstr(name, Xors("$ IDA trusted_idbs")) || strstr(name, Xors("$ IDA registry mutex $")))
	{
...
BOOL WINAPI DllMain(HMODULE hModule, DWORD Reason, LPVOID lpVoid) {
	if (Reason == DLL_PROCESS_ATTACH) {
		...
		utils::nt::library advapi32(Xors("advapi32.dll"));
		utils::nt::library user32(Xors("user32.dll"));
		utils::nt::library ntdll(Xors("ntdll.dll"));
		utils::nt::library kernel32(Xors("kernel32.dll"));
		utils::nt::library kernelbase(Xors("kernelbase.dll"));

		const char* main_module = Xors("BlackOpsColdWar.exe");
		g_Addrs.ModuleBase = (uintptr_t)(GetModuleHandle(0));
		nlog(Xors("Base Address: %p\n"), base);

		
		utils::hook::set(game.get_iat_entry(Xors("kernel32.dll"), Xors("QueryPerformanceCounter")), QueryPerformanceCounter_Detour);
		utils::hook::set(game.get_iat_entry(Xors("user32.dll"), Xors("SetWindowsHookExA")), SetWindowsHookExA_Detour);
...
void nlog(const char* str, ...) {
	...
	edit = FindWindowEx(notepad, NULL, Xors("EDIT"), NULL);
